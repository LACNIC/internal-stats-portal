...
class TagAPIListTestCase(FixtureTestCase):
    view_name = 'api-tags-list'
...
class TagAPIDetailTestCase(FixtureTestCase):
    view_name = 'api-tags-detail'

    def test_guest(self):
        url = reverse(self.view_name,
                      kwargs={'pk': self.tag_1_data['id']})
    ...
    def test_superuser_tags(self):
        ...
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, self.tag_1_data)

        # POST
        updated_field = {'name': 'a new name'}

        new_data = self.tag_1_data.copy()
        ...
        new_data['id'] = self.tag_1_data['id']
        self.assertEqual(response.data, new_data)
